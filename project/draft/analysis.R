...
cov_tab <- cbind(cov(data_gp[1:3]), cov(data2[1:3]), cov_mve, cov_mcd, cov_outmve, cov_outmcd, cov_promve, cov_promcd)
rownames(cov_tab) <- c("x1", "x2", "x3")
colnames(cov_tab) <- rep(paste0("x", 1:3), 8)

## factor loadings
ld_tab <- cbind(res1$lambda.aligned, res_adlp$lambda.aligned,
...
colnames(ld_tab) <- rep(paste0("x", 1:3), 8)
## intercepts
int_tab <- cbind(res1$nu.aligned, res_adlp$nu.aligned,
...
colnames(int_tab) <- rep(paste0("x", 1:3), 8)
