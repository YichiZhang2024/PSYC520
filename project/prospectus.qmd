...
The first step is to identify and categorize above-mentioned three type of data contamination. Methods using distances to measure how far an observation is from the center of the data cloud will allow us to achieve this purpose. Mahalanobis distance [@mahalanobis1936generalised] (MD) is one of them. However, the classic MD technique is not robust against outliers since it uses sample mean and covariance matrix. Robust estimates of location and scatter, such as Minimum Covariance Determinant [@rousseeuw1999fast] and Minimum Volume Ellipsoid [@rousseeuw1985multivariate] will be used to replace sample mean and covariance matrix in MD to calculate observations' distances. Once different types of data contamination are identified, Huber-type weights will be assigned to these data points, aiming to minimize their impact on model fit.
